...
Martin Wieskotten, Marielle Crozet, Bertrand Iooss, Céline Lacaux, Amandine Marrel. A comparison between Bayesian and ordinary kriging based on validation criteria: application to radiological characterisation. 2023. hal-03806713v2 (https://hal.science/hal-03806713v2/file/bayeskrig_clean.pdf)
...
We built two different Universal Kriging models to compare: one using the original data(standard model) and another using the logarithmic transformation(log model). To determine which model have better predictive performance, we employed the 'loo_compare' function to evaluate the fit of the models.The decision between the standard model and the log model allows us to find the suitable model to compare two Kriging models. 

The Expected Log Predictive Density(ELPD) is a measure of the model's predictive performance. A higher ELPD suggest better predictive accuracy.Our intial model used Universal Kringing and the given data itself yielded an ELPD estimate of -3039.1 with a standard error of 566.8, indicating some variability in the predictions. The alternative model, a logarithmic model using Universal Kringing showed a slightly better ELPD of -3028.9 with a standard error of 565.3, suggesting improved predictive performance. 

The effective number of parameters is a measure of model complexity, indicating how much the model has adapted to the data. In our initial model of Universal Kringing, this value was 383.7 with a standard error of 112.2. The alternative logarithm model used the same Universal Kringing had a slightly lower effective number of parameters at 372.9 with a standard error of 111.9, indicating that it might be less prone to overfitting.

The LOO information criterion(LOO-IC) combines the ELPD and the effective number of parameters to estimate the expected predictive error. A lower LOO-IC value indicates better performance. The initial model used Universal Kringin produced a LOO-IC of 6078.3 with a standard error of 1133.6, while the alternative model used Universal Kringin yielded a LOO-IC of 6057.7 with a standard error of 1130.6. The lower LOO-IC in the alternative model suggests it has slightly better predictive performance.

These results indicate that the alternative log model has slightly better predictive accuracy and lower complexity compared to the standard Universal Kriging model. However, the two model has a very close performance comparison. To further evaluate the models, we plan to compare the Universal Kriging and Bayesian models using the log transformation to see which approach provides more robust predictions.
