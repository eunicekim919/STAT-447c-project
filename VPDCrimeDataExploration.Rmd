...
crime_data_all <- crime_data[c(1:200),]
...
crime_data_sf <- st_as_sf(crime_data_all, coords = c("longitude", "latitude"), crs = 26710) # 26710 code for UTM10
...
ggplot() +
  geom_sf(data = area)+
  geom_sf(data = crime_data_sf, aes(color = MONTH)) +
...
#N1 is the number of data points (length(dfc[,1])) 
#x1 is the number of outcomes (dfc$x)
#n1 is the number of observations (dfc$n)
#N2 is the number of new points (length(fil[,1]))
#distance_matrix is the distances between all points

stan_data_pred <- list(N1 = nrow(crime_data),
                       x1 = crime_data$BINMONTH,
                       n1 = rep(1, 100),
                       N2 = nrow(crime_data_huh), 
                       distance_matrix = dist_matrix)
```
